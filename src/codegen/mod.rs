...
impl<'ctx> CodeGen<'ctx> {
    ...
    pub fn generate(&self, program: &CheckedProgram) {
        ...
        let printf_type = i32_type.fn_type(&[ptr_type.into()], true); // true for variadic
                ...
                if let CheckedExprKind::FunctionCall { name, args } = &expr.kind {
                    if name == "println" && args.len() == 1 {
                        ...
                        if let CheckedExprKind::StringLiteral(s) = &arg.kind {
                            ...
                            formatted.push('\n');
                            let global_str =
                                self.builder.build_global_string_ptr(&formatted, "str").unwrap();
                            self.builder
                                .build_call(
                                    printf_func,
                            ...
                            let global_str =
                                self.builder.build_global_string_ptr(fmt, "fmt").unwrap();
                            self.builder
                                .build_call(
                                    printf_func,
        ...
        match &expr.kind {
            ...
            CheckedExprKind::Binary { op, left, right } => {
                ...
                let res = match *op {
                    ...
                    BinaryOp::LtEq => self
                        .builder
                        .build_int_compare(inkwell::IntPredicate::SLE, l, r, "ltetmp")
                        .unwrap(),
                    BinaryOp::Gt => self
                        .builder
                    ...
                    BinaryOp::GtEq => self
                        .builder
                        .build_int_compare(inkwell::IntPredicate::SGE, l, r, "gtetmp")
                        .unwrap(),
